...
(() => {
  ...
  function spawnSlashEffect(stageIndex, x, y, dir = 1, scale = 1) {
  ...
  function applyHitToTarget(attacker, target, spec, dir) {
    ...
    setCharacterHp(target, (Number.isFinite(target.hp) ? target.hp : HP_MAX) - spec.damage);
  ...
  function drawCombatEffects() {
    const img = state.combat.slashFxImg;
    if (!img) return;
