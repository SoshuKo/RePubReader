...
(() => {
  ...
  function detachCarriedItem(carrier, dropToGround) {
...
function drawImageCover(img) {
  ...
  function handleAction(action, source = "system") {
    ...
    if (action === "roll") {
      ...
      actor.roll.dur = 0.62;
      actor.roll.dir = dir;
      actor.roll.targetRot = actor.rot + dir * Math.PI * 8;
      actor.vx = 0;
    ...
    if (action === "pickup") {
      ...
      if (actor.carryingItemId) {
        ...
        if (held && FOOD_ITEM_NAMES.has(held.name)) {
          startEating(actor, held, nowMs);
          setSpeechBubbleForEntity(actor.id, nowMs);
          return;
        }

        detachCarriedItem(actor, true);
        return;
      }
