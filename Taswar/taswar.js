...
function buildSeedPosts(accounts){const n=nowInfo(); const nowMin=n.d.getHours()*60+n.d.getMinutes(); const day=n.key; const y=new Date(n.d); y.setDate(y.getDate()-1); const yday=y.toISOString().slice(0,10); const out=[]; for(const a of accounts){for(const p of a.posts||[]){const tm=timeToMin(p.time); const useDay=(tm<=nowMin)?day:yday; out.push({id:`seed|${a.handle}|${useDay}|${p.time}|${(p.content||"").slice(0,36)}`,source:"seed",day:useDay,time:p.time,timestamp:`${useDay}T${p.time}:00`,name:a.name,handle:a.handle,icon:a.icon||{label:"U",color:"#00ffa1"},verified:!!a.verified,profile:a.profile||"",content:p.content||"",reply_to:p.reply_to||"",attachments:Array.isArray(p.attachments)?p.attachments:[]});}} return out;}
